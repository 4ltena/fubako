...
- **見た目**: 配布デザイン（Organic）を全画面に適用済み。色は読みやすさで詰め直してある（`npm run contrast` で検算。面は 1.2 以上、字は 4.5 以上、プレースホルダは 3 以上）。影を持たない面（伏せた紙・緑の帯・入力欄・丸いボタン）には縁を1本入れて、明暗どちらでも面が分かれるようにした。ダークモードは OS/ブラウザ追従のみ（トグルも設定行も作らない）。影は `@utility` にしてある（Tailwind v4 は `@theme` の `--shadow-*` を組み立て時に展開するので、`@media` で差し替えても効かない）
